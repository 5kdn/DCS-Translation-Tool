...
public class FileEntryServiceTests : IDisposable {
    ...
    private readonly TemporaryDirectory _temporaryDirectory = new( nameof( FileEntryServiceTests ) );
    ...
    public async Task GetChildrenRecursiveがファイルを含むディレクトリを指定した場合正しい結果が返る() {
        // Arrange
        var targetDir = Path.Combine( _temporaryDirectory.Path, "DirWithFile" );
    ...
    public async Task GetChildrenRecursiveは空のディレクトリを指定した場合空の結果が返る() {
        // Arrange
        var targetDir = Path.Combine( _temporaryDirectory.Path, "EmptyDir_" + Guid.NewGuid() );
    ...
    public async Task GetChildrenRecursiveは存在しないパスを指定した場合失敗結果を返す() {
        // Arrange
        var invalidPath = Path.Combine( _temporaryDirectory.Path, "NotExist_" + Guid.NewGuid() );
        var service = CreateService();
    ...
    public async Task GetChildrenRecursiveはWatch未実行でもハッシュキャッシュ初期化後に成功する() {
        // Arrange
        var targetDir = Path.Combine( _temporaryDirectory.Path, "DirectRecursiveWithRealCache" );
        Directory.CreateDirectory( targetDir );
        ...
        var preferredBaseDir = Path.Combine( _temporaryDirectory.Path, "cache-preferred" );
        var fallbackBaseDir = Path.Combine( _temporaryDirectory.Path, "cache-fallback" );
    ...
    public async Task GetChildrenRecursiveはハッシュキャッシュ初期化失敗時に失敗結果を返す() {
        // Arrange
        var targetDir = Path.Combine( _temporaryDirectory.Path, "CacheInitFailure" );
        Directory.CreateDirectory( targetDir );
        var filePath = Path.Combine( targetDir, "test.txt" );
        await File.WriteAllTextAsync( filePath, "Hello", TestContext.Current.CancellationToken );
        _hashCacheService
    ...
    public async Task Watchはファイルを追加したときEntriesChangedが発火する() {
        // Arrange
        var targetDir = Path.Combine( _temporaryDirectory.Path, "WatchDir" );
        Directory.CreateDirectory( targetDir );
        using var service = CreateService();
    ...
    public async Task Watchはハッシュキャッシュ初期化失敗時でも監視を継続する() {
        // Arrange
        var targetDir = Path.Combine( _temporaryDirectory.Path, "WatchWithCacheInitFailure" );
        Directory.CreateDirectory( targetDir );
        _hashCacheService
            .Setup( service => service.ConfigureRoot( targetDir ) )
            .Throws( new InvalidOperationException( "cache init failed" ) );
    ...
    public async Task Watchは存在しないディレクトリを指定したときGetEntriesAsyncが失敗結果を返す() {
        // Arrange
        var targetDir = Path.Combine( _temporaryDirectory.Path, "NotExistDir" );
        using var service = CreateService();
